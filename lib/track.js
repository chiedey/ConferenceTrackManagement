...
track.print = tracks => {
    tracks.forEach((track, idx) => {
        ...
        track.sessions.forEach(session => {
            ...
            if (limit != undefined) {
                let limitCost = time.duration(limit.noEarlier, limit.noLater);
                let finishBeginTime = '';

                if (session.timeRemain >= limitCost) finishBeginTime = time.militaryTimeTo12HrsClock(limit.noEarlier);
                else {
                    finishBeginTime = time.elapse(session.begin, session.timeUsed);
                    finishBeginTime = time.militaryTimeTo12HrsClock(finishBeginTime);
                }
